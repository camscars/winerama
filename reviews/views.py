...
def add_review(request, wine_id):
    ...
    if form.is_valid():
        ...
        review.save()
        update_clusters() #function from reviews/suggestions.py
...
def user_recommendation_list(request):
    ...
    # from the reviews, get set of wine IDs
    user_reviews_wine_ids = set(map(lambda x: x.wine.id, user_reviews))

    #get request user cluster name (only the first)
    try:
        user_cluster_name = \
            User.objects.get(username=request.user.username).cluster_set.first().name #this not required but speeds up query time.
    except:
        update_clusters()
        user_cluster_name = \
